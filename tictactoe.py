...
def print_board():  # function to print the board
    b = """
      ...
      3     {} |  {}  | {}
        """
    print(
        b.format(board[0], board[1], board[2], board[3], board[4], board[5],
...
def free_squares_temp(
        k):  # function which returns list of indices of free squares
    lst = []
    for i in range(len(k)):
        if (k[i] == " "):
            lst.append(i)
...
def special_case():
    if ((board[4] == xoro) & (([
            board[0], board[1], board[2], board[3], board[5], board[6],
            board[7], board[8]
    ]).count(xoro) == 1)):
        if (board[0] == xoro):
            return (2)
...
def next_move():
    ...
    # logic for special case of first corner move
        ...
        ([board[0], board[2], board[6], board[8]].count(xoro) == 1) &
        ([board[1], board[3], board[4], board[5], board[7]].count(" ") == 5)):
        return (4)
    ...
    # logic for special case of first edge move
        ...
        ([board[1], board[3], board[5], board[7]].count(xoro) == 1) &
        ([board[0], board[2], board[4], board[6], board[8]].count(" ") == 5)):
        return (4)
